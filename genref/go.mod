module github.com/kubernetes-sigs/reference-docs/genref

go 1.16

require (
	github.com/google/gofuzz v1.1.0
	github.com/kr/text v0.2.0 // indirect
	github.com/niemeyer/pretty v0.0.0-20200227124842-a10e7caefd8e // indirect
	github.com/pkg/errors v0.9.1
	github.com/tengqm/kubeconfig v0.0.0-20210707141017-7849c7b17290
	github.com/yuin/goldmark v1.3.6
	github.com/yuin/goldmark-highlighting v0.0.0-20210516132338-9216f9c5aa01
	gopkg.in/yaml.v2 v2.4.0 // indirect
	gotest.tools v2.2.0+incompatible // indirect
	k8s.io/api v0.22.0-rc.0
	k8s.io/apiextensions-apiserver v0.22.0-rc.0
	k8s.io/apimachinery v0.22.0-rc.0
	k8s.io/apiserver v0.22.0-rc.0
	k8s.io/client-go v0.22.0-rc.0
	k8s.io/cloud-provider v0.22.0-rc.0
	k8s.io/controller-manager v0.22.0-rc.0
	k8s.io/gengo v0.0.0-20201214224949-b6c5ce23f027
	k8s.io/klog/v2 v2.9.0
	k8s.io/kube-controller-manager v0.22.0-rc.0
	k8s.io/kube-proxy v0.22.0-rc.0
	k8s.io/kube-scheduler v0.22.0-rc.0
	k8s.io/kubelet v0.22.0-rc.0
	k8s.io/metrics v0.22.0-rc.0
	sigs.k8s.io/yaml v1.2.0

)
